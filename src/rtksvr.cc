...
#if 1 /* enabled to fix ephemeris hand-over problem (2011/6/21) */
...
extern int rtksvrinit(rtksvr_t *svr)
{
    ...
    for (i=0;i<3;i++) {
        if (!init_raw(svr->raw+i)||!init_rtcm(svr->rtcm+i)) {
            tracet(1,"rtksvrinit: init_raw/init_rtcm error\n");
            return 0;
        }
    }
                       ...
                       solopt_t *solopt, stream_t *moni)
{
    ...
    fprintf(stderr, "*****RTK server is up and running****\n");
    return 1;
}
/* stop rtk server -------------------------------------------------------------
* start rtk server thread
