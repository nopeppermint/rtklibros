...
static int startsvr(vt_t *vt)
{
    ...
    for (i=3;i<8;i++) {
        /*gridanie{*/
        // no longer check for file overwrite, just do it.
        if (strtype[i]==STR_FILE&&!confwrite_ros(vt,strpath[i])) return 0;
...
bool fetch_params_from_server(void)
{
...
bool restart_callback(rtklibros::SingleResponse::Request& request, rtklibros::SingleResponse::Response& response)
{
    ...
    // create fake cli args to feed to restart function
    char *fake_args[] = {"None"};
    int fake_narg = 1;
    cmd_restart(fake_args,fake_narg,vt_ptr);
    response.answer = "Restart request received.  Restarting rtklib server.";
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    ros::ServiceServer service = nh.advertiseService("rtk_restart", restart_callback);
    ros::ServiceServer service2 = nh.advertiseService("rtk_fetch_params", fetch_params_callback);
    ros::AsyncSpinner spinner(1); // this seems necessary to get the callbacks
    spinner.start();
    /*}end-gridanie*/
    ...
    bool check = false;
    if (!*file) {
        check = fetch_params_from_server();
    }
    if (!check){
